...
def rewrite() -> None:
    TOPIC_NAME = os.environ.get(
        "ASSET", "EURUSD"
    )  # override with ASSET env if you like
    SAMPLE_LIMIT = int(os.environ.get("SAMPLE_LIMIT", "15"))

    # Resolve topic_id
    t = run_cypher(
        "MATCH (t:Topic {name:$name}) RETURN t.id AS id", {"name": TOPIC_NAME}
    )
    ...
    for aid in ids:
        ...
        res = should_rewrite(
            topic_id, aid, test=False
        )  # returns dict: {'should_rewrite': bool, 'motivation': str, 'section': str}
        logger.info(f"Result: {res}")
        if res["should_rewrite"] is True:
            logger.info(
                f"Rewrite should have been triggered for section={res['section']} on topic={topic_id}"
            )
            triggered = True
            break
    ...
    if not triggered:
        logger.info(
            "No rewrite was triggered in this sample. Increase SAMPLE_LIMIT or try a different ASSET."
        )
