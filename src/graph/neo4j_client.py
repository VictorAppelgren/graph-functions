...
from .models import Neo4jRecord, Topic, Article, CountResult, IdResult, NodeExistsResult
    ...
    try:
        ...
        with driver.session(database="system") as sys_session:
            ...
            if NEO4J_DATABASE not in dbs:
                logger.warning(
                    f"Database '{NEO4J_DATABASE}' does not exist. Attempting to create it..."
                )
                sys_session.run(f"CREATE DATABASE {NEO4J_DATABASE}")
                logger.info(
                    f"CREATE DATABASE command issued for '{NEO4J_DATABASE}'. Waiting for database to be online..."
                )
                # ---
                ...
                while waited < max_wait:
                    ...
                    for r in dbs_info:
                        if r["name"] == NEO4J_DATABASE:
                            ...
                            break
                    logger.info(
                        f"Waited {waited:.1f}s: Database '{NEO4J_DATABASE}' status: {db_status}"
                    )
                    if db_status == "online":
                        break
                    time.sleep(interval)
                    ...
                    logger.error(
                        f"Database '{NEO4J_DATABASE}' did not become online after {max_wait}s."
                    )
                    raise RuntimeError(
                        f"Database '{NEO4J_DATABASE}' did not become online after {max_wait}s."
                    )
        ...
        logger.error(
            f"❌ Failed to connect to Neo4j or create database: {e}", exc_info=True
        )
        raise RuntimeError(f"Neo4j connection error: {e}")


def run_cypher(
    query: str, params: Optional[Dict[str, Any]] = None, database: Optional[str] = None
) -> List[Neo4jRecord]:
    """
...
def get_topics(
    ...
    Returns:
        List of Topic dictionaries with known structure
    """
    query = "MATCH (t:Topic)"
    params: Dict[str, Any] = {}
    ...
    return [cast(Topic, r["t"]) for r in results]
...
def get_topic_by_id(topic_id: str) -> Optional[Topic]:
    ...
    return cast(Topic, results[0]["t"]) if results else None


def get_articles(
    limit: Optional[int] = None, topic_id: Optional[str] = None
) -> List[Article]:
    """
    Get articles with strong typing.
    ...
    Returns:
        List of Article dictionaries with known structure
    """
    if topic_id:
    ...
    results = run_cypher(query, params)
    return [cast(Article, r["a"]) for r in results]
...
def node_exists(label: str, property_key: str, property_value: Any) -> bool:
    ...
    """
    query = (
        f"MATCH (n:{label} {{{property_key}: $value}}) RETURN count(n) > 0 as exists"
    )
