...
def rewrite_analysis_llm(
    ...
    logger.info(
        f"rewrite_analysis_initial | length={len(initial)} | preview={initial[:800]}"
    )
    ...
    logger.info(
        f"rewrite_analysis_critic_feedback | length={len(feedback)} | preview={feedback[:800]}"
    )
    ...
    logger.info(
        f"rewrite_analysis_source_checker | length={len(factual_corrections)} | preview={factual_corrections[:800]}"
    )
    ...
    return re.split(r"\n?Citations?:", final, flags=re.IGNORECASE)[0].rstrip()
