...
from src.graph.ops.topic import get_topic_id_by_name
...
def get_topic_id_and_analysis(topic_name: str) -> tuple[str, dict[str, str]]:
    ...
    if not analysis_fields:
        raise RuntimeError(
            f"No analysis fields found for topic '{topic_name}' (id={topic_id})"
        )
