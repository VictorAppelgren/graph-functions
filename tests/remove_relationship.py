...
def interactive_propose_and_remove():
    ...
    for n in nodes:
        ...
        candidate_links = [l for l in links if l.get('type') != 'ABOUT' and l.get('target') not in anchor_ids]
        ...
        print(f"Source candidate: {id_to_name.get(src, src)} ({src}), "
        ...
        source_node = {
            ...
            "type": id_to_node.get(src, {}).get("type", "Topic"),
        }
        candidate_ids = [l.get('target') for l in candidate_links if l.get('target')]
        candidate_motivation = "Non-ABOUT outgoing links from selected source (anchors excluded)."
        ...
        result = llm_select_link_to_remove(source_node, candidate_links, prioritized_link=prioritized_link)
        ...
        link = next((l for l in candidate_links if l.get('target') == target_to_remove), None)
        if not link:
        ...
        try:
            ...
            still = [l for l in after if l.get('type') == link.get('type')
