    ...
    async with httpx.AsyncClient(
        ...
        try:
            resp = await client.get(
                url, cookies={c["name"]: c["value"] for c in cookies or []}
            )
...
async def playwright_fetch(
    ...
    try:
        ...
        if login_cfg:
            try:
                await page.goto(
                    login_cfg.get("login_url", url),
                    wait_until="networkidle",
                    timeout=20000,
                )
                await random_delay()
                await page.fill(
                    'input[type="email"],input[name*="user"]', login_cfg["username"]
                )
        ...
        if is_article_good(text):
            logger.debug(
                f"Login+browser fetch succeeded ✅ ({len(text.split())} words)"
            )
            return text
        else:
...
async def scrape_article_and_sources(article_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for src_url in source_urls:
        ...
        if is_article_good(text):
            ...
            logger.debug(
                f"JSON length before adding source: {json_len_before}, after: {json_len_after}"
            )
