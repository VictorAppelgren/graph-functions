...
def aggregate_driver_analyses(topic_id: str) -> list:
    ...
    for d in influences:
        drivers.append({"name": d["name"], "relation": "influences", "executive_summary": d.get("executive_summary", "")})
    # CORRELATES_WITH
    correlates_query = """
    ...
    for d in correlates:
        drivers.append({"name": d["name"], "relation": "correlates_with", "executive_summary": d.get("executive_summary", "")})
    # PEERS
    peers_query = """
    ...
    for d in peers:
        drivers.append({"name": d["name"], "relation": "peer", "executive_summary": d.get("executive_summary", "")})
