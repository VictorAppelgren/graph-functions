"""
USER ANCHOR topicS AND RELATIONSHIPS

This file is part of the Saga Graph project—a world-scale knowledge graph for understanding macro, markets, and investment research. The core of this system is a massive Neo4j graph database, designed to represent the real structure and drivers of the world economy. All topics, relationships, and analyses are stored in Neo4j and can scale to hundreds of topics and thousands of articles per day. This file defines the permanent, user-controlled anchor topics and explicit relationships that serve as the foundation for all automated and LLM-driven reasoning in the graph.

HOW THIS STRUCTURE WORKS:
- Each anchor topic is a specific, atomic, standalone area of interest (e.g., EURUSD, S&P 500, US Inflation, Oil, Gold, Wheat).
- **General or group anchors (e.g., 'Commodities', 'Equities', 'Asia') are strictly prohibited.** Only specific, connectable entities are allowed. This is because:
    - Only specific topics can be meaningfully linked in a knowledge graph.
...
- Anchor topics are never deleted or overwritten by automated or LLM-driven processes. They are the pillars of your research universe.
- Anchor topics store minimal fields only: 'id', 'name', 'importance' (1..5 per `graph_topics/priority_policy.py`), and a 'query' string used for retrieval. No extra fields like status/type/notes.
- There is no parent-child hierarchy: all anchor topics are on the same level. Relationships between them are explicit and typed: 'INFLUENCES', 'CORRELATES_WITH', and 'PEERS' (rare).
- When building or refreshing the graph, these topics are always present. LLMs or other agents may add new topics or relationships, but never remove or modify anchors.
- You can update this file as your interests evolve—add, remove, or modify anchor topics and relationships at any time.
- This structure is designed for clarity, modularity, and reproducibility. It also provides a rich context for LLM prompts, ensuring that any automated reasoning or topic creation is always grounded in your real priorities and mental models.

EXAMPLE USAGE:
- On graph creation, load all anchor topics and relationships from this file and MERGE them into the graph.
- When prompting an LLM to propose new topics or relationships, provide anchor 'name' and 'query' as context.
...
USER_ANCHOR_TOPICS = [
    ...
    {
        "id": "case_shiller_20",
        "name": "Case-Shiller 20-City Home Price Index",
        "importance": 3,
    },
...
USER_ANCHOR_RELATIONSHIPS = [
    ...
    {
        "from": "china_credit_impulse",
        "to": "china_property_sales",
        "type": "INFLUENCES",
    },
    {
        "from": "china_manufacturing_pmi",
        "to": "china_exports",
...
def set_user_anchors():
    ...
    for topic in USER_ANCHOR_TOPICS:
        topic_id = topic["id"]
        logger.info(f"Processing anchor topic: {topic['name']} (id={topic_id})")

        # Check if topic already exists
        if check_if_topic_exists(topic_id):
            existing_topic = get_topic_if_exists(topic_id)
            logger.info(
                f"Anchor topic already exists: {existing_topic['name']} (id={topic_id})"
            )
            # Ensure importance is set to the configured value
            desired_imp = topic.get("importance")
            existing_imp = existing_topic.get("importance")
            if desired_imp is not None and existing_imp != desired_imp:
                run_cypher(
                    ...
                    {"id": topic_id, "importance": int(desired_imp)},
                )
                logger.info(
                    f"Updated importance for anchor {topic_id}: {existing_imp} -> {desired_imp}"
                )
            topics_existed += 1
            continue
    ...
    logger.info(
        f"Anchor topic seeding complete: {topics_created} topics created, {topics_existed} already existed."
    )
    ...
    if bad_rows:
        details = ", ".join(
            f"{r['id']} (importance={r.get('importance')})" for r in bad_rows
        )
    ...
    for rel in USER_ANCHOR_RELATIONSHIPS:
        ...
        try:
            ...
            logger.error(
                f"Error creating relationship {rel_type} from {source_id} to {target_id}: {str(e)}"
            )
            error_count += 1

    logger.info(
        f"Anchor relationships created: {success_count} successful, {error_count} errors"
    )
