...
def test_add_topic() -> None:
    ...
    day_dirs = [
        d
        for d in os.listdir(raw_base_str)
        if os.path.isdir(os.path.join(raw_base_str, d))
    ]
    ...
    for day in sorted(day_dirs, reverse=True):
        ...
        raise FileNotFoundError(
            f"No article JSONs found in any day folder under {raw_base_str}"
        )
    ...
    result = add_topic(article_id)
    print("[test_add_topic] Result:", result)
